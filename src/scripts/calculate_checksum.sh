...
if [[ -z "$CHANGESETIDENTIFIER" ]]; then
	echo "changesetIdentifier is required"
	exit 1
fi
if [[ -z "$URL" ]]; then
...
if [[ -n "$CHANGESETIDENTIFIER" ]]; then
	PARAMS+=("--changeset-identifier=$CHANGESETIDENTIFIER")
fi
...
docker-entrypoint.sh "${GLOBALS[@]}" calculate-checksum "${PARAMS[@]}"
